...
public class MainClass extends JavaPlugin
{
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args)
	{
		...
		else if(cmd.getName().equalsIgnoreCase("enchantstaff") && sender instanceof Player)
		{
			if(args.length == 2)
			{
				if(tryParseInt(args[1]))
				{
					AddStaffEnchantment((Player)sender, args[0], Integer.parseInt(args[1]));
					return true;
				}
			}
		}
	...
	public void AddStaffEnchantment(Player player, String ench, int level)
	{
		if(Staff.isStaff(player.getItemInHand()))
		{
			if(StaffEnchantmentType.isStaffEnchantmentType(ench))
			{
				Staff staff = new Staff(player, player.getItemInHand());
				StaffEnchantmentType type = StaffEnchantmentType.getStaffEnchantmentType(ench);
				StaffEnchantment Ench = new StaffEnchantment(type, Math.min(type.getMaxLevel(), level));
				staff.addStaffEnchantment(Ench);
			}
			else 
				player.sendMessage(ChatColor.RED + Msgs.get("unknownenchantment"));
		}
		else
			player.sendMessage(ChatColor.RED + Msgs.get("notstaffinhand"));
	}
