...
public class StaffsLoader 
{
	...
	public static void GetConfig(JavaPlugin plugin) throws IOException
	{
		...
		for(List<String> staff : staffsinfo)
		{
			try	{
				...
				Bukkit.getLogger().warning("Error occured while reading staff " 
	...
	public static void GenerateStaff(List<String> staff) throws IOException
	{
		...
		boolean hasRecipe = HasRecipe(staff);
		ShapedRecipe recipe;
				
		//saving information into tag
		NBTTagCompound tag = new NBTTagCompound();
		tag.setString("staffType", type);
		tag.setFloat("staffDamage", (float) damage);
		tag.setFloat("staffCastTime", (float) casttime);
		tag.setInt("staffCurrentDurability", durability);
		tag.setInt("staffMaxDurability", durability);
		tag.setInt("staffMaxDistance", distance);
